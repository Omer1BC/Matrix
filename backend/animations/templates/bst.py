...
class BSTVisualizer(VGroup):
    ...
    def delete(self, value, run_time=5):
        ...
        if has_two_children:
            ...
            mark_for_deletion = pred_circle.animate.set_fill(RED, opacity=0.8).set_run_time(
                1.2
            )
            ...
            revert_color = target_circle.animate.set_fill(GREEN, opacity=0.6).set_run_time(
                1.0
            )
    ...
    def _delete_node(self, node, value):
        ...
        if value < node.value:
            ...
            if node.label:
                ...
                if max_node.value in self.node_mobjects:
                    del self.node_mobjects[max_node.value]
                self.node_mobjects[node.value] = (node.circle, node.label)
            node.left = self._delete_node(node.left, max_node.value)
        return node
